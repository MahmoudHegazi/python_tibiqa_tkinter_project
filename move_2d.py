...
def moveChamp(event):
    ...
    if event.keysym.lower() == 'right':
        ...
        if champ_move_x < (cWidth - champ_size):
            champ_move_x += champ_size 
            w.moveto(mychamp, champ_move_x, champ_move_y)
            print('>Right Dim (X): ' , champ_move_x)
            character_actions.append("right")        
    elif event.keysym.lower() == 'left':
        if champ_move_x > 0:
            champ_move_x -= champ_size 
            w.moveto(mychamp, champ_move_x, champ_move_y)
            print('<left Dim (X): ' , champ_move_x)
        ...
        if champ_move_y > 0:
            champ_move_y -= champ_size
            w.moveto(mychamp, champ_move_x, champ_move_y)
            print('^UP Dim (Y): ' , champ_move_y)
            character_actions.append("up")
    elif event.keysym.lower() == 'down':
        if champ_move_y < (cHeight - champ_size):
            champ_move_y += champ_size
            w.moveto(mychamp, champ_move_x, champ_move_y)
            print('^Down Dim (Y): ' , champ_move_y)
