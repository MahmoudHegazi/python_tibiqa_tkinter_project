# in this update compas object update when champ moves detect every posible square around champ can move into or cast spell or attack or have monster on it
# with new prepre update for diagonally cx,cy = x1, y1+square = ncx,ncy
from tkinter import *
import random
import keyboard
import time
...
def updateCompasY(y, obj_size):
    ...
    if checkup >= 0:
        # up to
        print("can have monster or attack from UP side ^")
    ...
    if event.keysym.lower() == 'right':
        ...
        if champ_position['x'] < (const_last_right):
            ...
            if checkright <= const_last_right:
                ...
                print("can have monster or attack from Right side >")
            else:
        ...
        if champ_position['y'] > 0:
            ...
            updateCompasY(champ_position['y'], champ_size)
